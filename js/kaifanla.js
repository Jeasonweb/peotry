 ...
 *制作日期： 2016-08-08 11:48:33
 **/
//大的模块的注释方式
//***************************
//功能描述:通过配置文件方式实现页面跳转的路径配置
//****************************/
var app=angular.module('myApp',['ng','ngRoute','ngAnimate']);
    ...
    app.controller('mainCtr',function($scope,$http){
        ...
        $scope.loadMore = function(){
            $http.get('data/dish_getbypage.php?start='+$scope.dishList.length).
                success(function(data){
                    if(data.length<5){  //当服务器返回的菜品数量不足5条,dang
                        $scope.hasMore = false;
                    }
    ...
    app.controller('orderCtr',function($scope,$http,$routeParams,$rootScope){
        //console.log($routeParams.dishid);
    ...
    app.controller('myorderCtr',function($scope,$http,$rootScope){
